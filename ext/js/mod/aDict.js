...
getWords, merge, chrome, unconjugate, japaneseUtil
Analyze */
var aA
var av = (v) => {
...
var lg = (...v) => {
  if (av('log')) {
    ...
    // Silently log (hidden from the page)
    hiddenLogs.push(...v);
  }
};
var hiddenWarns = [];
...
var wa = () => {
  console.w(...hiddenLogs, ...hiddenWarns)
  return [hiddenLogs, hiddenWarns]
}
console.log = lg
console.warn = wr
if (av('warn')) {
  console.warn = console.w;
}
if (av('log')) {
...
class Dict {
  constructor(display = null, audio = null, japaneseUtil = null, anki = null, control = null, mecab = null, param = true) {
    ...
    const nSizes = (w = 1.2, h = 1.4) => {
      ...
      height = this.limit
      width = this.wid
      if (av('warn')) console.warn(width, height)
    }
    ...
    var pop
    var jpmn
    // Define the ankis function
    var ankisAll = async (op = 0) => {
      def = aDeck('Def', 'asc')
      anm = aDeck('Anm', 'asc')
      jp2 = aTag('jp2', 'asc')
      pop = aTag('inPopup', 'asc')
      jpmn = aModel('JP Mining Note', 'asc')
      await Promise.all([def, anm, jp2, jpmn, pop])
        .then((results) => {
          const [def, anm, jp2, jpmn, pop] = results
          // Process the results
          if (av('log')) console.log([def, anm, jp2, jpmn, pop])
          let local = localStorage.getItem('words')
          localStorage.setItem('wordsBackup', local)
          local = local.split(' ')
          let jpws = getWords(jpmn.notes)
          let anms = getWords(anm.notes)
          let pops = getWords(pop.notes)
          const all = jpws // getWords(jp2.notes)
          const un = merge(local, jpws)
          aNote.svDiv(document.querySelector('.save'), un, undefined, true)
    ...
    /*
        this.txtImg(true)
  .then((image) => {
    if(av('log')) console.log('Image:', image);
  })
  .catch((error) => {
...
this.txtImg(false)
  .then((text) => {
    if(av('log')) console.log('Text:', text);
  })
  .catch((error) => {
    ...
    if (this.search) {
      ...
      if (av('log')) console.log(tx)
      tx.onkeyup = (e) => {
        if (av('log')) console.log(e, document.querySelector('#search-textbox').value)
                ...
                if (m >= 0) {
                  try {
                    if (lc[m].innerHTML.length > 0) {
                      ...
                      if (lc[m].tagName == 'H2') {
                        if (av('log')) console.log(lc[m].innerText)
                      ...
                      try {
                        ...
                        if (av('log')) console.log(m);
                      }
                      // if(av('log')) console.log(m, lc[m].innerHTML, iz, lz, lt, ad)
                    }
                  } catch (esx) {
                    if (av('log')) console.log(esx)
                  }
                  m += 1
                }
                //for (const {} of org) {                  // lng.innerHTML += ll                }
              }
            } catch (le) {
              console.error(le)
            }
        ...
        if (l == 330) {
          ...
          for (const ijk in lis) {
            if (ijk >= 0) {
              ...
              // if(av('log')) console.dir(lis[i])
              if (av('log')) console.dir(tll)
              for (const i in tll) {
    ...
    if (this.search || this.page) {
      ...
      this.keys = [
        ...
        'numpad8',        // 59 - 104
        'numpad9',        // 60 - 105
        'numpadmultiply', // 61 - 106
        'numpadadd',      // 62 - 107
        'numpadsubtract', // 63 - 109
        'numpaddecimal',  // 64 - 110
        'numpaddivide',   // 65 - 111
        'f1',             // 66 - 112
        'f2',             // 67 - 113
        'f3',             // 68 - 114
        ...
        'f12', // 77 - 123
        'contextmenu', // 78 - 93
        'audiovolumedown', // 79 - 174
        'audiovolumemute', // 80 - 173
        'audiovolumeup', // 81 - 175
        'mediaplaypause', // 82 - 179
        'mediastop', // 83 - 178
        'mediaprevioustrack',// 84 - 177
        'medianexttrack', // 85 - 176
        'osleft', // 86 - 91 (left Windows key)
        'osright', // 87 - 92 (right Windows key)
        'browsersearch', // 88 - 170
        'browserhome', // 89 - 172
        'launchmail', // 90 - 180
        'launchapp1', // 91 - 182
        'launchapp2', // 92 - 183
      ...
      const f2 = async function (w, from, to) {
        ...
        for (const i of a) {
          if (i.length > 0 && i.indexOf(' --> ') == -1 && !(i.length < 10 && parseInt(i) >= 0)) {
            ...
            if (av('warn')) console.warn(i.length)
            r += i + '\n'
          }
        }
        return r
      ...
      if (document.getElementById('export')) {
        ...
        document.getElementById('export').oncontextmenu = async () => {
          ...
          for (const el in v) {
            if (av('warn')) console.warn(`${el}: Exporting`, v[el])
            await sv(v[el], -2)
          ...
          fileInput.addEventListener('change', async (e) => {
            ...
            if (file) {
              try {
                ...
                // Import the data to localStorage
      ...
      this.lq = ''
      this.kr()
      if (av('log')) console.log(this.search, this.page, spl, txt)
      ...
      if (av('log')) console.log(spl[this.istart], this.istart)
      ...
      if (!document.querySelector('.save')) {
        ...
        let sb = this.btc('Words-JPMN', async function (e) {
          e.stopPropagation()
          let si = localStorage.getItem('words') // + ` ${localStorage.getItem('wordbk')}`*/ document.querySelector('.save .w').innerText
          si = si.split(' ')
          this.svs(si, await this.jpws)
          this.hke = true
        }.bind(this), saveDiv)
        this.btc('Words-Anki All Cards', async function (e) {
          e.stopPropagation()
          let si = localStorage.getItem('words') // + ` ${localStorage.getItem('wordbk')}`*/ document.querySelector('.save .w').innerText
          si = si.split(' ')
          wn(si)
          this.svs(await this.cards, si)
          this.hke = true
        }.bind(this), saveDiv)
        this.btc('Words-Popups', async function (e) {
          e.stopPropagation()
          let si = localStorage.getItem('words') // + ` ${localStorage.getItem('wordbk')}`*/ document.querySelector('.save .w').innerText
          si = si.split(' ')
        ...
        this.btc('JPMN', async function (e) {
          e.stopPropagation()
          wn(e)
          const si = await this.jpws //localStorage.getItem('jpmns') // merge(null,null, 0)
          wn(si)
          this.svs(si, [], 90)
          this.hke = true
        }.bind(this), saveDiv)
        this.btc('-SentenceAudio', async function (e) {
          e.stopPropagation()
          wn(e)
          const si = await this.saudio //localStorage.getItem('jpmns') // merge(null,null, 0)
          wn(si)
          this.svs(si, [], 90)
          this.hke = true
        }.bind(this), saveDiv)
        this.btc('+SentenceAudio', async function (e) {
          e.stopPropagation()
          wn(e)
          const si = await this.query(`SentenceAudio:[* -tag:inSearch`) //localStorage.getItem('jpmns') // merge(null,null, 0)
          wn(si)
          this.svs(si, [], 90)
          this.hke = true
        }.bind(this), saveDiv)
        this.btc('Popups', async function (e) {
          e.stopPropagation()
          wn(e)
          const si = await this.pops //localStorage.getItem('jpmns') // merge(null,null, 0)
          wn(si)
          this.svs(si, [], 90)
          this.hke = true
        }.bind(this), saveDiv)
        this.btc('Anms', async function (e) {
          e.stopPropagation()
          wn(e)
          const si = await this.anms //localStorage.getItem('jpmns') // merge(null,null, 0)
          wn(si)
          this.svs(si, [], 90)
          this.hke = true
        }.bind(this), saveDiv)
        this.t.insertAdjacentElement('afterend', saveDiv)
        try {
          // let sy = localStorage.getItem('save')
          // sy = JSON.parse(sy);
          // sav += s[0] + ' ';
          // for (let s of sy) {
          // }
          //let s2 = saveDiv.appendChild(document.createElement('div'))
          //aNote.displayObjectInHTML(sy, s2, aNote)
      ...
      if (av('warn')) console.warn(this.lmt)
      if (av('warn')) console.warn(performance.memory.usedJSHeapSize) // how much you're currently usin
      ...
      let qpc = document.querySelector('#query-parser-container')
      ...
      document.querySelector('#istart').onchange = (e) => {
        ...
        if (av('warn')) console.warn(e, input)
        const iid = input.id
        const lg = input.value
        if (av('log')) console.log(e, lg)
        const v = lg
        if (lg === null) {
  ...
  async yomi(spl, i, fast = true) {
    ...
    if (this.first && document.querySelector('.query-parser-segment-reading')) {
      ...
      for (const p of pd) {
        this.prev.appendChild(p.cloneNode(true))
      }
    }
    if (av('log')) console.log(spl, i)
    if (this.search) {
      this.lq = spl?.join('\n') ?? this._display.fullQuery
    } else {
      this.lq = this.v
    }
    this.frst = false
    ...
    try {
      ...
      if (av('log')) console.dir(document.querySelectorAll('div>span.query-parser-term.newline'))
      // if(av('log')) console.log(`Search mode ${document.querySelector(".content-body-inner")}`)
      // let mode = 'lyrics'
      // let spl = txt
      // .filter(item=>item);
      if (av('log')) console.dir(spl)

      const rm = document.querySelectorAll('.entry')
      //for (const r in rm) {try {              // rm[e].parentElement.removeChild(rm[e]);            } catch { }          }
      ...
      // if(av('log')) console.dir(this.modK)
      ...
      // run = false
      if (av('warn')) console.warn(fast, window.navigator.onLine)
      /*if (this.jpdb instanceof Promise && this.fval > 0) {
        if(av('warn')) console.warn('awaiting ', this.jpdb)
        await this.jpdb
      }*/
      this.lines = []
      ...
      if (fast && window.navigator.onLine) {
        ...
        const cc = document.querySelector('#query-parser-content').children
        this.parse(cc)
        await this.runner(spl, cc, true)
        this.first = false
      }
      // if(av('log')) console.log(isKana)
    } catch (e) {
      if (av('log')) console.log(e)
    }
  ...
  async makeElem(tt, spl, ii, splL, I, w, line, emph, ltp, tts, mns = null, y = null, elem = null, o = this.var) {
    ...
    const isPart = this.var('pt') ? ptc : false
    ...
    try {
      ...
      if (ltpChildren.length === 0) {
        ...
        const pattern2 = new RegExp(`class="word">${searchString}<rt`, 'g')
        if (av('warn')) console.warn(I, searchString, pattern2, firstChildText)

        const ts = pattern2.test(firstChildText)
        if (!ts) {
          let hasReplaced = 0;
          let replacedText = firstChildText.replace(pattern, (match, index) => {
            if (hasReplaced == 0) {
              let iel = isIndexInsideElement(firstChild, index);
              if (!iel) {
                //hasReplaced = 1;
    ...
    if ((f < this.fval && false) || (isPart) || (tt.length < this.min)) {
  ...
  async moe(spl, ii, splL) {
    ...
    try {
      if (this.start && ii >= 0) {
        ...
        const ttx = tt.split(/[^ -~]+/)
        if (av('warn')) console.warn('ttttt', tt, ttx)
        const ix = ii
        if (av('log')) console.log(`${parseInt(ii) + 1}/${spl.length}.  : `, ii, tt, tts, ix)
        if (spl[ii] >= 0) {
          this.nl = true
          this.nll = true
          if (av('log')) console.log(`===-=>${this.nl}`)
        }
        const il = tt.length
        if (av('warn')) console.warn(il)
...
<link rel="stylesheet" type="text/css" href="./moe.css">`
        ...
        if (document.querySelector('.vis')) {
          if (document.querySelectorAll('.vis').length < 1 && this.var('eras')) {
  ...
  async loadMore(istart, st = null) {
    ...
    this.stop = true
    // spl = sgroup
    const aff = this._display.fullQuery.split('\n')
    let sgroup = this.groupByN(this.prt, aff, istart)
    ...
    if (av('warn')) console.warn(this.spl, sgroup, ss, istart, this.var('slw'))
    localStorage.setItem('navsentence', ss)
    // this.modP.parentNode.removeChild()
    this.pos = 0
    await this.yomi(ss, istart, true) //.bind(this) // this.var('slw'))
    if (this.pos == 0 && st != null && !this.var('yc')) {
      const b = document.querySelectorAll('.vis')
      const p = st ? 0 : (this.var('del') ? b.length - 1 : this.pos - 1)
  ...
  async runner(sp, t, done, modP = this.modP) {
    //debugger
    let emph = false
    let line = []
    let ptxt
    ...
    let ind = 0
    let w = []
    if (av('warn')) console.warn(cc.children)
    ...
    for (const ii in c) {
      ...
      try {
        if (ii >= 0) {
          ...
          if (done) {
            ...
            if (av('warn')) console.warn(c[ii], tts, tt)
            if (tt == '') {
              tt = c[ii].innerText
            }
            s += tt.length
            if (av('warn')) console.warn(tt, tt.length, Y, l[Y], qs[Y], s)
            if (s > l[Y]) {
              s = 0
              Y += 1
              pf()
          ...
          if (isKana) {
            ...
            if (av('warn')) console.warn(results, performance.now())
            if (results?.length > 0) {
              let yc = []
              const fq = -1
              const fqs = []
              ...
              if (!done) {
                if (av('warn')) console.warn(yc, results, tt)
                return
              }
              if (!this.var('yc')) {
              ...
              if (av('warn')) console.warn(bot[0].innerHTML, bot[3].innerHTML, yc)
              const y = bot[3].innerHTML + bot[0].innerHTML
              const retv = await this.makeElem(tt, c, ii, c.length, this.I, w, line, emph, ptxt, tts, null, y)
              emph = retv[0]
              line = retv[1]
              w = retv[2]
            }
      ...
      try {
        if (this.lang == 'id' && this.I > 2) {
          let tt = this._display.fullQuery.toLocaleLowerCase()
          const tls = this.modT.appendChild(document.createElement('div'))
          tls.id = 'mtl'
          const tll = tls.appendChild(document.createElement('div'))
          const mk = tll.appendChild(document.createElement('span'))
  ...
  async input(e) {
    ...
    let ki = -1
    for (ki in this.keys) {
      // if(av('log')) console.log(keys[ki], e.key);
      if (e.key.toLowerCase() == this.keys[ki]) {
        ret = false
        break
      }
    }
    ...
    if (ki <= 9) {
      ...
      if (n > 0) {
        ...
        let ec = n == 90 ? b.length - 1 : null
        eb = ec || Math.round(n * b.length / 100)
      } else {
        eb = 0
      }
      try {
      ...
      const mv = (d, pos, _t, b) => {
        ...
        if (d == -1) {
          try {
            if (av('warn')) console.warn(pos, l, b[l].parentElement.children.length, b[l].parentElement.children[0])
          } catch (error) {
            return -1
          }
          let m = 0
          ...
          while (m < b.length) {
            m += 1
            lm = l - m
            if (av('warn')) console.warn(`Iteration: ${m}, lm: ${lm}, l: ${l}, b[lm]: ${b[lm]}`) // Log iteration details
            try {
              if (!b[l]?.parentNode?.contains(b[lm])) {
                if (av('warn')) console.warn('Element not contained in parent') // Log when element is not contained in parent
      ...
      if (this.var('rd')) {
        if (b[this.pos].querySelector('.query-parser-segment-text')) {
          try {
            const t = b[this.pos].querySelector('.query-parser-segment-text') ? b[this.pos].querySelector('.query-parser-segment-text').innerText : b[this.pos].innerText
            if (av('warn')) console.warn(t)
            let x = -1
            if (av('warn')) console.warn(t, this.posr)
            for (const y of ys) {
              let z
              if (b[this.pos].querySelector('.query-parser-segment-text')) {
      ...
      if (ki >= 17 && ki <= 18) {
        ...
        this.istart += 1 * id
        this.loadMore(this.istart, id == 1)
      }
      if (ki == 22) {
      ...
      if (e.key == '' || e.key == ' ') {
        if (b[this.pos]) {
          if (b[this.pos].style.height.length > 1) {
            b[this.pos].style.height = ''
            b[this.pos].style.flex = 'none'
          } else {
            b[this.pos].style.height = localStorage.getItem('ht')

            b[this.pos].style.flex = localStorage.getItem('wt')
          }
        }
        if(this.pos < 0){
        this.pos = 0
        ...
        b[this.pos].scrollIntoView()// {behavior: 'auto',block: 'center'});
        this.cache(b[this.pos], this.istart, this.pos)
      }
      wn(ki)
      if (ki == 21) {
      ...
      if (kn == ';') {
        let si = localStorage.getItem('words') // + ` ${localStorage.getItem('wordbk')}`*/ document.querySelector('.save .w').innerText
        si = si.split(' ')
        this.svs(si, await this.cards)
        this.hke = true
      }
      if (kn == "]") {
        let si = localStorage.getItem('words') // + ` ${localStorage.getItem('wordbk')}`*/ document.querySelector('.save .w').innerText
        si = si.split(' ')
        this.svs(await this.jpws, si)
        this.hke = true
      }
      }
      /* if(kn == 'unidentified'){
        let si = localStorage.getItem('words')
        si = si.split(' ')
        this.svs(await this.anms, si)
        this.hke = true
      }*/
    }
  }
  svs(si, ws = [], ln = this.svl, cp = false) {
  ...
  get bcc() {
    return (async () => {
      if (typeof this._bcc !== 'undefined' && this._bcc !== null) {
        return this._bcc
      }
      return fetch('bcc.json')
        .then((response) => response.json())
        .then((jsonData) => {
          // Use the jsonData object here
          this._bcc = jsonData
          if (av('log')) console.log('bcc---------', jsonData)
          return this._bcc
        })
        .catch((error) => {
  ...
  get cc() {
    return (async () => {
      if (typeof this._cc !== 'undefined' && this._cc !== null) {
        return this._cc
      }
      return fetch('cc.json')
        .then((response) => response.json())
        .then((jsonData) => {
          // Use the jsonData object here
          this._cc = jsonData
          if (av('log')) console.log('cc---------', jsonData)
          return this._cc
        })
        .catch((error) => {
          console.error('Error fetching JSON:', error)
  ...
  get jpdb() {
    return (async () => {
      ...
      return fetch('jpdb.json')
        .then((response) => response.json())
        .then((jsonData) => {
          // Use the jsonData object here
          this._jpdb = jsonData
          if (av('log')) console.log('jpdb---------', jsonData)
          return this._jpdb
        })
        .catch((error) => {
          console.error('Error fetching JSON:', error)
  ...
  async ankis(op = 0) {
    ...
    switch (op) {
      case 0:
        var def = await aDeck('Def', 'asc');
        return def;
      case 1:
        var anm = await aDeck('Anm', 'asc');
        return anm;
      case 2:
        var jp2 = await aTag('jp2', 'asc');
        return jp2;
      case 3:
        var pop = await aTag('inPopup', 'asc');
        return pop;
      case 4:
        var jpmn = await aModel('JP Mining Note', 'asc');
        return jpmn;
      default:
        var all = await aAll('asc')
        return all
    }
  }
  async update(q, from = null) {
    if (av('warn')) console.warn(q)
    this.new = true
    if (this.var('cp') === false) {
      this._display = null
      return
    }
    if (this.var('rl')) {
    ...
    if (this.var('run')) {
      ...
      this.istart = -1
      this.spl = fq.split('\n')// .filter(item=>item);
      const ln = this.spl.length
      const nl = this.int('lim')
      if (this.prt > nl) {
        localStorage.setItem('lim', this.prt)
      } else if (this.prt < (nl / 5)) {
        localStorage.setItem('lim', Math.round(nl / 5))
      }
      this.lis = ln > nl
      this.len = ln
      this.spl = [this.spl]
      if (this.var('slw')) {
        this.read = false
        let txt = fq
        console.log(this.spl, ln, nl)
        if (this.var('hk') || this.lis) {
          this.istart = 0
          this.spl = this.groupByN(this.prt, this.spl[0], this.istart)
          await this.yomi.bind(this, this.spl[this.istart], this.istart)()
        } else {
          await this.yomi.bind(this, this.spl[this.istart + 1], this.istart + 1)()
        }
      } else {
  ...
  async wait() {
    ...
    if (av('log')) console.log('the wait is over!')
  }

  async autoClick() {
    await this.waitFor((_) => document !== null)
    await this.waitFor((_) => this.ac() == true)
    // if(av('log')) console.log('0!');
    await new Promise((r) => setTimeout(r, 400))
    // if(av('log')) console.log(document.querySelector('#dictionary-entries > div.entry.entry-current > div.entry-header > div.actions > button:nth-child(4) > span') !== null);
    if (this.ac()) {
      this.anki._addAnkiNote(0, 'term-kanji')
    }
  }

  async find(k, optionsContext, i, full) {
    const p = []
    ...
    for (let j = i + 4; j >= i; j--) {
      ...
      // p.push(pr)
      // if(av('log')) console.log(i,j,pr);
    }
    if (av('log')) console.log(p)
    return true
  }
  btc(textContent, onClickListener, ct = null) {
    const button = document.createElement('button');
    button.textContent = textContent;
    button.addEventListener('click', onClickListener);
    if (ct) {
      ct.appendChild(button)
    }
    return button;
  }
  int(id) {
...
<!--                    <div class="settings-item-description">${description}</div>-->
    ...
    if (av('log')) console.log(inputs)
    function setupToggleSwitch(container, localStorageKey, _fallbackValue) {
    ...
    function generateTemplate(value, bgColor, size, border, label, def = 1) {
      ...
      try {
        ...
        if (av('warn')) console.warn(idOrName, def)
        let b = ''
        if (def != 1) {
    ...
    const toggleButton = (button, key) => {
      if (av('log')) console.log(button, key)
      let value = false
      try {
        value = localStorage.getItem(key) === 'true'
      } catch (vr) {
        value = false
        if (av('warn')) console.warn(key)
    ...
    for (let i = 0; i < 8; i++) {
      try {
        ...
        const buttonElement = document.createElement('div')
        if (av('warn')) console.warn(i, buttonElement, label)
        buttonElement.innerHTML = buttonTemplate
        // const button = buttonElement //.firstChild;
        // Add the button to the btn array
        inputs.appendChild(buttonElement)
    ...
    if (av('log')) console.dir([inputs, btn])
    return [inputs, btn]
  }

  hide(i) {
    if (av('log')) console.log(i)
    let q
  ...
  async dictionary(k = false, optionsContext, i, t = 0, _r = null, _ln = null, _nl = null, elem) {
    ...
    try {
      ...
      // let prnt = null
      if (av('log')) console.log('==>' + this.nl, this.var('yc'), this.var('fq'))
      if (sc) {
      ...
      if (this.wiki) {
        ...
        if (z.length > 1) {
          ...
          // if(av('log')) console.dir(all)
          // if(av('log')) console.log(z, st)
      ...
      if (!this.pe || !this.kan) {
        ...
        for (const i in x) {
          ...
          // _ ==== _
          //                    if(av('warn')) console.warn(c, cx, x[i]);
          if (cx >= 0) {
            ...
            if (av('log')) console.log(kd, kdis)
            //for (const { } in kdis) {try { /* empty */ } catch (iuy) {console.error(iuy)}
          }
        }
      }
      let W
  ...
  async g(w, from = 'auto', to, api, maxLength = 5000) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // if(av('warn')) console.warn(translated);
      if (api == 0) {
    ...
    // if(av('warn')) console.warn(chunks, translatedChunks, r, maxLength);
    return r
  }
  ...
  tl(w, from = 'auto', to, api) {
    ...
    // if(av('warn')) console.warn(api, url);
    if (this.stop) {
      return
    }
  ...
  async web(url, o, kj) {
    ...
    if (av('log')) console.log(`WebUrl: ${url}`)
    if (kj === false) {
      // if (kj.length > 1) {
      return
      // }
    ...
    if (av('log')) console.dir(url)
    if (av('log')) console.dir(promise)
    // const timeout = setTimeout(() => controller.abort(), 5000);
    const html = await promise.text()
    if (o == 0) {
      // if(av('log')) console.dir(kj)
      const h = document.createElement('p')
  ...
  delete(el = null) {
    try {
      this.spl = []
      if (document.querySelector('#modP')) document.querySelector('#modP').remove()
      this.modP = null
      this.startMod()
    } catch (ee) {
      console.error(ee);
  ...
  startMod() {
    ...
    this.modP = document.createElement('div')
    if (av('warn')) console.warn(this.modP)
    const ps = this.var('pre') ? ['afterend', document.querySelector('#dictionary-entries')] : ['beforebegin', document.querySelector('#dictionary-entries')]
    if (!ps[1]) {
      ps[1] = document.body
    }
    ...
    if (av('warn')) console.warn(ps)
    return ps
  }

  cache(b, istart, pos = -1, load = null) {
    if (!pos) {
      pos = b.getAttribute('pos')
    }
    let ch = localStorage.getItem('navcache') ?? ''
    if (load) {
      localStorage.setItem('navsave', `${istart}:${pos}:${this.spl?.join('\n').substring(0, 80)}`)
      ch += `${localStorage.getItem('navsave')}`
    } else {
      localStorage.setItem('navsave', `${istart}:${pos}:${b.getAttribute('w')}`)
      const pb = b.parentElement?.getAttribute('txt')
      const pi = b.parentElement?.getAttribute('ti')
      localStorage.setItem('navsentence', pb)
      ch += `${pi}.${localStorage.getItem('navsave')}: ${localStorage.getItem('navsentence')}`
    }
    ch = ch.substring(ch.length - 800)
    localStorage.setItem('navcache', ch)
    this.note?.svDiv(document.querySelector('.save'))
  }

  async originate(z, h, _optionsContext) {
    const RGEX_CHINESE = new RegExp(/[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff]/)
    ...
    await Promise.all([
      ...
      try {
        ...
        if (av('warn')) console.warn(lu, h, lc)
        let lt = false
        let ad = false
        let or = false
        for (const m in lc) {
  ...
  async frequency(word, dbs = this.jpdb) {
    ...
    const entry = await db.find((entry) => entry[0] === word || entry[2].reading === word) ?? -1
    return (entry[2]?.frequency?.value ? entry[2]?.frequency?.value : entry[2]?.frequency) ?? -1
  }

  async reading(word, dbs = this.jpdb) {
    let db = await this.is(dbs)
    const entry = db.find((entry) => entry[0] === word)
    return entry ? entry[2].reading : null
  }
  async is(v) {
    if (v instanceof Promise) {
      if (av('warn')) console.warn('awaiting ', v)
      await v
      if (av('warn')) console.warn('done ', v)
    }
    return v
  }
  kr() {
    ...
    initialjamo["t'"] = 16
    initialjamo['t’'] = 16
    if (av('warn')) console.warn(initialjamo, this.lang)
    ...
    const maxlen = 3
    if (av('warn')) console.warn(this.lang)
    if (av('warn')) console.warn(document.querySelector('#search-textbox').value)
    if (this.lang == 'ko') {
        ...
        try {
          for (const y of yc) {
            ...
            if (this.var('fq')) {
              ...
              for (const f of fqe) {
                if (av('warn')) console.warn(f.parentElement.parentElement)
              ...
              const ji = RGEX.test(tt)
              if (!ji) {
                // fq /= 4
              } else {
                fq *= 175 / 100
              }
              if (av('warn')) console.warn(fqs, ji, fq, fq < parseInt(localStorage.getItem('freq')))
  ...
  prevent(l) {
    ...
    // if(av('warn')) console.warn(l, this.lmt, performance.memory.usedJSHeapSize);
