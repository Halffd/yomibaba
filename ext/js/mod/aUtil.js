...
function isIndexInsideElement(node, index) {
    if (!node || index < 0) {
        return false;
    }
...
async function aIn(t = '', anki = aDict?.cards){
    let arr = merge(aNote.get(), await anki, -1, false)
    let r = arr?.res?.includes(t)
    wn(arr, r)
    return r
}
function merge(local = null, jpws = null, rev = -1, str = true) {
    if (!local) {
...
async function aAll(sort = false) {
    const r = true
    while (r) {
        try {
 ...
 * conjugate a verb
 */
var conjugate = function (verb, type) {
    type = type || "v5";
 ...
 */
var unconjugate = function (word, verbtype) {
