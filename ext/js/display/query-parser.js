...
/* global
 * TextScanner EventDispatcher yomichan log av
 */
...
class QueryParser extends EventDispatcher {
    ...
    _getParseResult() {
        ...
        if(av('qp')) console.log(selectedParser, this._parseResults.find((r) => r.id === selectedParser))
    ...
    _renderParseResult() {
        ...
        if(av('qp')) console.dir(parseResult)
    }
                ...
                if (reading.length === 0) {
                    try {
                        if(this._japaneseUtil.isStringEntirelyKana(text)){
                            //termNode.style.letterSpacing = '1.05rem'
                            let r0 = this._japaneseUtil.convertKatakanaToHiragana(text)
                            let r1 = this._japaneseUtil.convertToRomaji(r0)
                            termNode.appendChild(this._createSegment(text, r1, offset));
                        } else {
                            termNode.appendChild(document.createTextNode(text));
                        }
                    } catch (e) {
                        if(av('qp')) console.log(e)
                        termNode.appendChild(document.createTextNode(text));
                    }
                } else {
