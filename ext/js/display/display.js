...
class Display extends EventDispatcher {
    ...
    async _setContentTermsOrKanji(type, urlSearchParams, token) {
        ...
        if (document.URL.includes('search.html') && document.URL.includes('chrome-extension')) {
            run = localStorage.getItem('run') == 'true' ? true : false;
        }
        //console.log(run, document.URL);
        if (!run || !(document.URL.includes('search.html') && document.URL.includes('chrome-extension'))) {
    ...
    _setQuery(query, fullQuery, queryOffset) {
        ...
        if (typeof aDict !== 'undefined' && aDict !== null) {
            aDict.update(query)
        }
    }
    ...
    _updateQueryParser() {
        ...
        var run = false
        if (document.URL.includes('search.html') && document.URL.includes('chrome-extension')) {
            var qp = localStorage.getItem('qp') == 'true'
            run = localStorage.getItem('run') == 'true' ? true : false;
            run = run && !qp
        }
        console.log(run, document.URL);
